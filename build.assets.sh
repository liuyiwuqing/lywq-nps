...
tar -czvf build/windows_amd64_client.tar.gz npc.exe conf/verification.key -C conf/bat ./


CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 go build -ldflags "-s -w -extldflags -static -extldflags -static" ./cmd/npc/npc.go

tar -czvf build/darwin_amd64_client.tar.gz npc -C conf/sh ./
